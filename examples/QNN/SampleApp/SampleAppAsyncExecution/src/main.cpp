...
void showHelp() {
  std::cout
      ...
      << "  --save_context      <VAL>       Specifies that the backend context and metadata "
         ...
         "                                  the same time.\n"
      << "\n"
      << "  --sync                          Run inferences synchronously.\n"
         "                                  Default - Runs inference asynchronously\n"
      << "\n"
...
std::unique_ptr<sample_app::QnnSampleApp> processCommandLine(int argc,
  ...
  enum OPTIONS {
    ...
    OPT_SAVE_CONTEXT     = 12,
    OPT_SYNC             = 13,
    OPT_NUM_INFERENCES   = 14,
    OPT_VERSION          = 15,
    OPT_SYSTEM_LIBRARY   = 16
  };
  ...
  static struct pal::Option s_longOptions[] = {
      ...
      {"sync", pal::no_argument, NULL, OPT_SYNC},
...
int main(int argc, char** argv) {
  ...
  if (app->isAsyncMode()) {
    if ((status == EXIT_SUCCESS) && sample_app::StatusCode::SUCCESS != app->executeGraphsAsync()) {
      status = app->reportError("Graph Execution failure");
    }
  } else {
    if ((status == EXIT_SUCCESS) && sample_app::StatusCode::SUCCESS != app->executeGraphsSync()) {
      status = app->reportError("Graph Execution failure");
    }
  }
