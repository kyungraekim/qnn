...
iotensor::IOTensor sample_app::QnnSampleApp::getIOTensor() { return m_ioTensor; }

void sample_app::QnnSampleApp::incrementNumThreadsCallback() { num_threads_callback += 1; }

// #define MAXIMUM_FRAMES 10
// typedef struct FrameTimings {
//     uint64_t start_graph;
//     uint64_t ready_graph;
//     uint64_t done_graph;
//     uint64_t done_output;
// } FrameTimings;
...
// FrameTimings frameTimes[MAXIMUM_FRAMES];
unsigned int totalFrames = 0;

// extern std::shared_ptr<::qnn::log::Logger> g_logger;
// void recordTimeForCurrentFrame(enum Timepoint timepoint, unsigned int frameNumber) {
//     uint64_t now = g_logger->getTimestamp();
//     switch (timepoint) {
//         case START_GRAPH:
//             frameTimes[frameNumber].start_graph = now;
//             break;
//         case READY_GRAPH:
//             frameTimes[frameNumber].ready_graph = now;
//             break;
//         case DONE_GRAPH:
//             frameTimes[frameNumber].done_graph = now;
//             break;
//         case DONE_OUTPUT:
//             frameTimes[frameNumber].done_output = now;
//             break;
//     }
// }

void sample_app::asyncCallback(void* notifyParam, Qnn_NotifyStatus_t notifyStatus) {
  // capture the elapsed time since the inference was enqueued to report total time in profile data
  if (!notifyParam) {
    QNN_ERROR("Async notify param pointer is NULL");
    return;
  }
  auto args =
      *static_cast<std::pair<sample_app::QnnSampleApp*, asyncInferenceMetadata>*>(notifyParam);
  asyncInferenceMetadata c_inferenceMetadata = args.second;
  iotensor::IOTensor ioTensor                = args.first->getIOTensor();
  QNN_INFO("Done execution for graph: %d, start write output\n", c_inferenceMetadata.graphIdx);

  // save the outputs
  if (iotensor::StatusCode::SUCCESS !=
      ioTensor.writeOutputTensors(c_inferenceMetadata.graphIdx,
                                  c_inferenceMetadata.inputFileIndexOffset,
                                  c_inferenceMetadata.graphName,
                                  c_inferenceMetadata.outputs,
                                  c_inferenceMetadata.numOutputs,
                                  c_inferenceMetadata.outputDatatype,
                                  c_inferenceMetadata.graphsCount,
                                  c_inferenceMetadata.outputPath,
                                  c_inferenceMetadata.numInputFilesPopulated,
                                  c_inferenceMetadata.outputBatchSize)) {
    return;
  }
  ioTensor.tearDownInputAndOutputTensors(c_inferenceMetadata.inputs,
                                         c_inferenceMetadata.outputs,
                                         c_inferenceMetadata.numInputs,
                                         c_inferenceMetadata.numOutputs);
  args.first->incrementNumThreadsCallback();
  QNN_INFO("Done graph %d\n", c_inferenceMetadata.graphIdx);
}

sample_app::StatusCode sample_app::QnnSampleApp::executeGraphsAsync() {
  // load the input data
  auto returnStatus = StatusCode::SUCCESS;
  std::list<std::pair<sample_app::QnnSampleApp*, asyncInferenceMetadata>> callbackArgs;

  QNN_INFO("Running %u inferences in asynchronous mode... \n", m_numInferences);

  totalFrames = m_inputFileLists.size();

  for (unsigned int run = 0; run < m_numInferences; run++) {
    for (size_t inputIdx = 0; inputIdx < totalFrames; ++inputIdx) {
      QNN_INFO("Start graph %d\n", inputIdx);
      for (size_t graphIdx = 0; graphIdx < m_graphsCount; graphIdx++) {
        QNN_DEBUG(
            "Starting execution for graphIdx: %d, m_graphsCount=%zu\n", graphIdx, m_graphsCount);
        ...
        auto inputFileList = m_inputFileLists[inputIdx];
          ...
          while (inputFileIndexOffset < totalCount) {
            ...
            if (iotensor::StatusCode::SUCCESS !=
                m_ioTensor.setupInputAndOutputTensors(
                    &inputs, &outputs, (*m_graphsInfo)[graphIdx])) {
            ...
            std::tie(iotReturnStatus, numInputFilesPopulated, batchSize) =
                m_ioTensor.populateInputTensors(inputIdx,
                                                inputFileList,
            ...
            if (iotensor::StatusCode::SUCCESS != iotReturnStatus) {
              QNN_ERROR("Error in populateInputTensors");
              returnStatus = StatusCode::FAILURE;
            }
            // add all the required data into inferenceMetadata
            inferenceMetadata.inputs                 = inputs;
            inferenceMetadata.graphIdx               = inputIdx;
            inferenceMetadata.inputFileIndexOffset   = inputFileIndexOffset;
            inferenceMetadata.graphName              = graphInfo.graphName;
            inferenceMetadata.outputs                = outputs;
            inferenceMetadata.numInputs              = graphInfo.numInputTensors;
            inferenceMetadata.numOutputs             = graphInfo.numOutputTensors;
            inferenceMetadata.outputDatatype         = m_outputDataType;
            inferenceMetadata.graphsCount            = m_graphsCount;
            inferenceMetadata.outputPath             = m_outputPath;
            inferenceMetadata.numInputFilesPopulated = numInputFilesPopulated;
            inferenceMetadata.outputBatchSize        = batchSize;
            auto it =
                callbackArgs.emplace(callbackArgs.begin(), std::make_pair(this, inferenceMetadata));
            // execute the graph
            if (StatusCode::SUCCESS == returnStatus) {
              QNN_DEBUG(
                  "Successfully populated input tensors for graphIdx: %d, numInputTensors=%d, "
                  "numOutputTensors=%d",
                  graphIdx,
              ...
              executeStatus =
                  m_qnnFunctionPointers.qnnInterface.graphExecuteAsync(graphInfo.graph,
                                                                       inputs,
                                                                       graphInfo.numInputTensors,
                                                                       outputs,
                                                                       graphInfo.numOutputTensors,
            ...
            inputFileIndexOffset += inferenceMetadata.numInputFilesPopulated;
          }
        }
      }
      QNN_DEBUG("%s: Current number of active graph executions: %zu.\n", __func__, num_threads);
    }
  } /* loop numInferences */

  while (true) {
    if (num_threads <= num_threads_callback) {
      QNN_DEBUG("Exiting Async execute, timings:");
      break;
    }
  }
...
sample_app::StatusCode sample_app::QnnSampleApp::executeGraphsSync() {
  auto returnStatus = StatusCode::SUCCESS;

  QNN_DEBUG("Running %u inferences in synchronous mode... \n", m_numInferences);

  for (unsigned int run = 0; run < m_numInferences; run++) {
    for (size_t graphIdx = 0; graphIdx < m_graphsCount; graphIdx++) {
      QNN_INFO("Starting execution for graphIdx: %d", graphIdx);
      if (graphIdx >= m_inputFileLists.size()) {
        QNN_ERROR("No Inputs available for: %d", graphIdx);
        returnStatus = StatusCode::FAILURE;
        break;
...
bool sample_app::QnnSampleApp::isAsyncMode() { return m_asyncMode; }