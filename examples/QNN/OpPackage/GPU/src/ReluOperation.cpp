...
ReluOperation::ReluOperation(const QnnGpuOpPackage_Node_t* node, Qnn_ErrorHandle_t* status)
  ...
  const Qnn_Tensor_t input        = QNN_GPU_OP_CFG_GET_INPUT_TENSOR(*(node->configs[0]), 0u);
  const uint32_t inputRank        = QNN_GPU_TENSOR_GET_RANK(input);
  uint32_t* const inputDimensions = QNN_GPU_TENSOR_GET_DIMENSIONS(input);
  ...
  const Qnn_Tensor_t output        = QNN_GPU_OP_CFG_GET_OUTPUT_TENSOR(*(node->configs[0]), 0u);
  const uint32_t outputRank        = QNN_GPU_TENSOR_GET_RANK(output);
  uint32_t* const outputDimensions = QNN_GPU_TENSOR_GET_DIMENSIONS(output);
